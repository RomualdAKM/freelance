...
const formationId = ref(route.params.formationId)

const getFormation = async () => {
    let response = await axios.get(`/api/get_formation/${formationId.value}`)
...
formData.append("title", formation.value.title);
formData.append("description", formation.value.description);

if (formation.value.picture) {
    formData.append("picture", formation.value.picture);
}


await axios.post("/api/edit_formation/" + formation.value.id, formData).then((response) => {
...
if(response.data.success){
            ...
            console.log('ok')
              toast.fire({
                  icon: "success",
                  title: "formation modifier avec success",
              });
